...
if [ "$1" == "ALL" ]; then
  ...
  for i in `./lib/getLanguages.sh`; do ./lib/createLinks.sh "$i" >> "$filename.files.txt"; done
  for i in `cat "$filename.files.txt"`; do cat "$i" >> "$filename"; done
  ...
  for i in `cat "$filename.files.txt"`; do wc -l "$i" >> "$filename.stats.txt"; done
...
if [ "$2" == "BIGMEM" ]; then
  ...
  sort -S 50% --field-separator=$'\t'  -k2 -k1 --temporary-directory=. -no "$filename"".right" "$filename"
